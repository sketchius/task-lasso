...
import { assignTasks } from '../data/task-manager';
...
export default function ToDoList(navigation, route) {
	...
	return (
		<View style={styles.container}>
			{status == 'CHECK-IN' ? (
				<View>
					{store.getState().summaryDate && (
						<View>
							<StyledText style={styles.summaryHeaderText}>
								Summary for {formatRelative(store.getState().summaryDate, new Date())}
							</StyledText>
							<StyledProgressBar
								sections={[
									{
							...
							<View style={[styles.alignedRow, styles.marginLeft5]}>
								...
								<StyledText style={[styles.summaryTaskText, styles.tealText]}>
									{completedTasks} TASKS COMPLETED
								</StyledText>
							</View>
							<View style={[styles.alignedRow, styles.marginLeft5]}>
								{getIcon('AntDesign', 'arrowright', 30, styles.colors.blue)}
								<StyledText style={[styles.summaryTaskText, styles.blueText]}>
									{deferredTasks} TASKS DEFERED
								</StyledText>
							</View>
							<View style={[styles.alignedRow, styles.marginLeft5]}>
								{getIcon('MaterialCommunityIcons', 'emoticon-sad-outline', 30, styles.colors.red)}
								<StyledText style={[styles.summaryTaskText, styles.redText]}>
					...
					<SelectionList
						styles={styles}
						data={'ambition'}
						label={'HOW IS YOUR MOTIVATION TODAY?'}
						columns={3}
						selection={ambition}
						onPress={setAmbition}
						orientation={'row'}
					...
					<Pressable
						style={[styles.screenHeaderButton, styles.marginBottom6]}
						onPress={() => handleCheckIn()}>
						<StyledText>Start the day!</StyledText>
					</Pressable>
				</View>
			) : (
				<ScrollView style={styles.container}>{toDoListItems()}</ScrollView>
			)}
		</View>
	);
}
