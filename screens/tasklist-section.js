...
export default function TasklistSection(props) {
	const filteredTaskList = useSelector(state =>
		state.tasks.filter(task =>
			props.type == 'COMPLETED' ? task.status == 1 : task.type === props.type && task.status != 1
		)
	);
	...
	const getTaskListItems = () => {
		return filteredTaskList.map((task, i) => (
			<TodoItem key={task.uniqid} styles={styles} task={task} navigation={props} tasklist={true} />
		));
	};

	const getSectionTaskCountElement = () => {
		return (
			<Text style={[styles.defaultText, styles.circleBorder, styles.taskCountElement]}>
				{filteredTaskList.length} ITEMS
			</Text>
		);
	};
			...
			<View style={[styles.alignedRow, styles.topBorders]}>
				<View style={[styles.sectionHeaderStyle, styles.alignedRow]}>
					...
					<StyledText
						style={[
							...
							filteredTaskList.length == 0 && styles.gray3Text,
						]}>
						{props.label}
					</StyledText>
				</View>
				...
				{filteredTaskList.length > 0 && (
					<View style={styles.caret}>
						{getIcon('AntDesign', expanded ? 'caretdown' : 'caretleft', 16, styles.colors.gray3)}
					</View>
				)}
			</View>
			<View style={[styles.topBorder, expanded && styles.bottomBorder]}>
